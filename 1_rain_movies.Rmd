...
#use line below to explore different color palettes
...
```{r}
#putting all of the temperature values into 1 col
long_daily_p <- pivot_longer(daily_p1, c(mean_temp, max_temp, min_temp), names_to = "temp")

#taking the station names and creating value columns for each station
station_daily_p <- pivot_wider(long_daily_p, names_from = station, values_from = value)

#grouping by station to filter out NAs
aff <- station_daily_p %>%
  select(1,5,6) %>%
  filter(!is.na(AFF))
...
#joining filtered stations
fs_daily_p <- inner_join(aff, fnl, by = c('date','temp'))


p3 <- ggplot(fs_daily_p,aes(x = FNL,y = AFF, color = temp)) + 
  geom_line() +
  geom_point() +
  labs(title = '100 mile temp diff 10/18-10/19', x='FNL Temp(f)',y= 'AFF Temp (f)') +
  transition_reveal(date) + 
  scale_color_few()

animate(p3, nframes=(nrow(fs_daily_p)))
...
#basemap for the creation of GIF below
basemap1 <- tm_shape(co) + 
    tm_polygons() + 
  tm_shape(co_elev) +
    tm_raster(style='cont',
              palette = '-viridis',
...
#not sure if this generation works as I coulnd't get it to generate within a reasonable time
#on my laptop ¯\_(ツ)_/¯ 
saveGIF({
  for(i in sequence){
  sub_sf <- daily_sf %>%
  ...
  if(nrow(sub_sf) > 0){
  finalmap <- basemap1 + tm_shape(sub_sf) + 
    tm_dots(title.size='Daily Precip (in)',
            col='lon',
            size='daily_p',
            style='cont',
